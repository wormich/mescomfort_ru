...
class FormQuestionsList
{
    ...
    private function getFormData($elementData = [])
    {
        ...
        $answerFormData = [
            ...
            'ADDRESS_OBJECT' => (!empty($data['ADDRESS_OBJECT'])) ? $data['ADDRESS_OBJECT'] : current($arAnswer2['comment'])['USER_TEXT'],
        ];
...
//            );
    ...
    private static function getFormQuestionListHtml($arFields = [])
    {
        ...
        <?php foreach ($arFields as $key => $section) { ?>
        <tr class="heading">
            <td colspan="2"><?= $key ?></td>
        </tr>
        <?php foreach ($section as $propertyKey => $property) { ?>
            <tr>
                <?php if ($property['FIELD_TYPE'] != 'hidden') { ?>
                    <td><?= $property['NAME'] ?></td>
                <?php } ?>
                <td <?php if (!empty($property['DESCRIPTION'])) { ?>style="display: flex;    align-items: center;"<?php } ?>>
                    <div <?php if (!empty($property['DESCRIPTION'])) { ?>style="width: 160px;"<?php } ?>>
                        <?php
                        switch ($property["FIELD_TYPE"]) {
                            ...
                            case "radio":
                                foreach ($property['LIST_PARAMS'] as $PARAM) {
                                    ...
                                    echo $input;
                                    echo "<label for=\"" . $PARAM['CODE'] . "\">" . htmlspecialcharsbx($PARAM["NAME"]) . "</label><br />";
                                }
                                break;
                            case "checkbox":
                                foreach ($property['LIST_PARAMS'] as $PARAM) {
                                    ...
                                    echo $input . "<label for=\"" . $property['FIELD_NAME'] . "\">" . htmlspecialcharsbx($PARAM["NAME"]) . "</label><br />";
                                }
                                break;
                            case "dropdown":
                                $prop = [];
                                ...
                                if ($property['FIELD_NAME'] == 'QUIZ[COMPLECT_NAME]') {
                                    ...
                                    <select class="inputselect" name="form_dropdown_<?= $property['FIELD_NAME'] ?>" id="COMPLECT_NAME" style="width: 61.5%">
                                        <?php foreach ($property['LIST_PARAMS'] as $prop) { ?>
                                            <option value="<?= $prop['CODE'] ?>" data-fem="<?= $prop['FEM'] ?>" <?php if ($prop['CODE'] == $property['VALUE']) { ?>selected<?php } ?> data-akb="<?= $prop['AKB'] ?>" data-type="<?= $prop['TYPE_NAME'] ?>"><?= $prop['NAME'] ?></option>
                                        <?php } ?>
                                    </select>
                                <?php } else {
                            ...
                            case "image":
                                if (!empty($property["VALUE"]))
                                {
                                    foreach ($property['VALUE'] as $img) {
                                        ...
                                        {
                                            echo \CFile::ShowImage($img, 200, 200, "border=0 class=\"printable\"", "", true);
                                            echo "<br />";
                                            echo '<input type="hidden" name="' . $property['FIELD_NAME'] . '[]" value="' . $img . '">';
                                            echo '<input type="checkbox" value="Y" name="form_image_'.$property['FIELD_NAME'] .'_del[' . $img . ']" id="form_image_'.$property['FIELD_NAME'] .'_del[' . $img . ']" /><label for="form_image_'.$property['FIELD_NAME'] .'_del[' . $img . ']">'.GetMessage('FORM_DELETE_FILE').'</label>';
        ...
        if (!empty(current($arAnswer2['subservices'])['ANSWER_VALUE'])) {
            ...
            if ($element = $el->fetch()) {
                ...
                FormResultAnswerTable::update($resultId['ID'], [
                    ...
                    'USER_TEXT_SEARCH' => ($element['SECTION_NAME']) ? mb_strtoupper($element['SECTION_NAME']) : 'ВЫЗОВ ЭЛЕКТРИКА НА ДОМ',
                ]);
