...
CAdminFileDialog::ShowScript(
	...
	<tr>
		...
		<td valign="top" width="60%"><?
			if (!isset($IBLOCK_ID))
				$IBLOCK_ID = 0;
			echo GetIBlockDropDownListEx($IBLOCK_ID, 'IBLOCK_TYPE_ID', 'IBLOCK_ID',array('CHECK_PERMISSIONS' => 'Y','MIN_PERMISSION' => 'W'), "", "", 'class="adm-detail-iblock-types"', 'class="adm-detail-iblock-list"');
		?></td>
	</tr>
	<?
}
...
/*		if (!empty($arSectionProps))
	...
	{
		?><tr>
			<td valign="top" width="40%"><b><? echo GetMessage("CATI_FIELD"); ?> <? echo $i+1; ?></b> (<? echo htmlspecialcharsbx(TruncateText($arDataFileFields[$i], 15)); ?>):</td>
			<td valign="top" width="60%">
				...
				foreach ($arAvailFields as &$arOneAvailField)
				{
					...
					?><option value="<? echo htmlspecialcharsbx($arOneAvailField['value']); ?>" <? echo (!empty($arOneAvailField['STYLE']) ? 'style="'.$arOneAvailField['STYLE'].'"' : ''); ?> <? if (${"field_".$i}==$arOneAvailField["value"] || !isset(${"field_".$i}) && $arOneAvailField["value"]==$arDataFileFields[$i]) echo "selected"; ?>><?echo htmlspecialcharsex($arOneAvailField["name"]); ?></option><?
				}
				?>
				</select>
			</td>
	...
	<tr>
		<td valign="top" width="40%"><? echo GetMessage("CATI_IMG_PATH"); ?>:</td>
		<td valign="top" width="60%">
	...
	<tr>
		<td valign="top" width="40%"><label for="IMAGE_RESIZE_Y"><? echo GetMessage("CATI_IMG_RESIZE"); ?></label>:</td>
		<td valign="top" width="60%">
	...
	?>
	<tr>
		<td valign="top" width="40%"><label for="USE_TRANSLIT_Y"><? echo GetMessage('CATI_USE_CODE_TRANSLIT'); ?></label>:</td>
		<td valign="top" width="60%">
	...
	{
		?><tr>
			<td valign="top" width="40%"><? echo GetMessage('CATI_CODE_TRANSLIT_LANG'); ?>:</td>
			<td valign="top" width="60%">
	...
	<tr>
		...
		<td valign="top" width="60%"><?
			if (!isset($outFileAction) || empty($outFileAction))
				$outFileAction = 'H';
	...
	<tr>
		...
		<td valign="top" width="60%"><?
			if (!isset($inFileAction) || empty($inFileAction))
				$inFileAction = 'F';
...
if ($STEP < 4)
{
	...
	if ($STEP > 2)
	{
		...
		$arfieldsString = array(
			'IBLOCK_ID','URL_DATA_FILE','fields_type','delimiter_r','delimiter_other_r','first_names_r','metki_f','first_names_f','PATH2IMAGE_FILES','USE_TRANSLIT','TRANSLIT_LANG','IMAGE_RESIZE','outFileAction','inFileAction','max_execution_time'
		);
		for ($i = 0, $intCountDataFileFields = count($arDataFileFields); $i < $intCountDataFileFields; $i++)
		{
