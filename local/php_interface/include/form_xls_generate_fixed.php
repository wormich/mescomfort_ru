...
$filename = "form_result_list_fixed.xlsx";
...
if (CheckFilter())
{
    if ($F_RIGHT >= 20)
    {
        $arFilter = Array(
            ...
            "STATUS_ID"					=> $_REQUEST['find_status_id'],
            "STATUS_ID_EXACT_MATCH"		=> $_REQUEST['find_status_id_exact_match'],
            "TIMESTAMP_1"				=> $_REQUEST['find_timestamp_1'],
            "TIMESTAMP_2"				=> $_REQUEST['find_timestamp_2'],
            "DATE_CREATE_1"				=> $_REQUEST['find_date_create_1'],
            "DATE_CREATE_2"				=> $_REQUEST['find_date_create_2'],
            "REGISTERED"				=> $_REQUEST['find_registered'],
            "USER_AUTH"					=> $_REQUEST['find_user_auth'],
            "USER_ID"					=> $_REQUEST['find_user_id'],
            "USER_ID_EXACT_MATCH"		=> $_REQUEST['find_user_id_exact_match'],
            "GUEST_ID"					=> $_REQUEST['find_guest_id'],
            "GUEST_ID_EXACT_MATCH"		=> $_REQUEST['find_guest_id_exact_match'],
            "SESSION_ID"				=> $_REQUEST['find_session_id'],
            "SESSION_ID_EXACT_MATCH"	=> $_REQUEST['find_session_id_exact_match']
        );
        if (is_array($arFormCrmLink))
...
while($arRes = $result->NavNext(true, "f_")) {
    ...
    foreach ($arrAnswers as $arAnswer) {
        ...
        if (in_array($answ['SID'], $arVisible)) {
            ...
            if (stripos($answ['USER_TEXT'], 'http') !== false) {
                $answ['USER_TEXT'] = htmlspecialchars($answ['USER_TEXT']);
            }

            $answer[$propertyKey] = $answ['USER_TEXT'] ? $answ['USER_TEXT'] : $answ['ANSWER_TEXT'];
        }
    }
    ksort($answer);
